...
// Show the end game
    ...
    <body>
        <div class="container">
            <div class="nes-container is-rounded">
                <img src="static/cthulhu.jpeg" style="width: 100%;">
                Thou shalt not live!
            </div>
        </div>
    </body>
    ...
    <body>
        <div class="container">
            <div class="nes-container is-rounded">
                <img src="static/cthulhu_lose.jpeg" style="width: 100%">
                You have defeated Cthulhu with your power. Take your flag: polyuctf{7H3_s@NItY_CHEck_Pa5SEd}.
            </div>
        </div>
    </body>
    EOD;

    if (hash('sha256', $_POST['payload']) !== $_POST['checksum']) 
        die('Not for you, dishonest hacker!');
    ...
    foreach($actions as $action) {
        ...
        if ($player_sanity <= 0 || $cthulhu_sanity <= 0) 
            die('Not for you, dishonest hacker!');
        if ($attacker !== 'C' && $attacker !== 'P') 
            die('Not for you, dishonest hacker!');
        if (!preg_match_all('/^\d{3}$/', $check_roll)) 
            die('Not for you, dishonest hacker!');
        if (!preg_match_all('/^\d{3}$/', $delta_roll)) 
            die('Not for you, dishonest hacker!');
        ...
        if ($attacker === 'C') {
            if ($player_sanity < $check_roll && $delta_roll > 20) 
                die('Not for you, dishonest hacker!');
            elseif ($player_sanity >= $check_roll && $delta_roll > 6) 
                die('Not for you, dishonest hacker!');
            $player_sanity -= $delta_roll;
        } else {
            if ($cthulhu_sanity < $check_roll && $delta_roll > 20) 
                die('Not for you, dishonest hacker!');
            elseif ($cthulhu_sanity >= $check_roll && $delta_roll > 6) 
                die('Not for you, dishonest hacker!');
